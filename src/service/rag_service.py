...
from ingestion import initialize_rag, index_file, direct_llm_query
...
def _is_no_context(text: str) -> bool:
    lower = text.lower()
    return any(phrase in lower for phrase in _NO_CONTEXT_PHRASES)
    ...
    async def get_answer(self, question: str, mode: str = "mix", top_k: int = 5, force_reindex: bool = False) -> str:
        ...
        if self.rag is not None:
            ...
            try:
                ...
                if result is not None and not _is_no_context(result):
                    return result
                print("RAG returned no relevant context, falling back to direct LLM...")
                return await direct_llm_query(question)
            except Exception as e:
